...
MIMIC-III Waveform + sepsis3 cohort CSV →
Sepsis 예측용 환자 단위 train/test .pt 파일 생성.

라벨: sepsis3 (0=non-sepsis, 1=sepsis)

Mortality task와 동일한 구조이되, 라벨이 sepsis3이다.
교차 일반화 스토리: 한국 데이터(VitalDB) pretrain → 미국 데이터(MIMIC-III) 평가.

사용법:
    python -m downstream.organ_dysfunction.sepsis.prepare_data \
        --cohort-csv downstream/classification/sepsis/bquxjob_93e3c7c_19d8f609070.csv \
        --waveform-dir datasets/raw/mimic3-waveform-sepsis \
        --out-dir datasets/processed/sepsis \
...
def prepare_dataset(
    ...
    """Sepsis prediction 데이터셋 준비 (환자 단위 그룹핑)."""
    ...
    print(f"  Sepsis+: {sum(1 for p in patients.values() if p['sepsis3']==1)}")
    print(f"  Sepsis-: {sum(1 for p in patients.values() if p['sepsis3']==0)}")
    ...
    if max_patients is not None:
        pos_sids = [s for s in subject_records if patients[s]["sepsis3"] == 1]
        neg_sids = [s for s in subject_records if patients[s]["sepsis3"] == 0]
    ...
    n_pos = sum(1 for p in patient_data if p["sepsis3"] == 1)
    n_neg = sum(1 for p in patient_data if p["sepsis3"] == 0)
    total_windows = sum(len(p["windows"]) for p in patient_data)
    avg_windows = total_windows / len(patient_data)
    print(f"\nPatients: {len(patient_data)} (Sepsis+={n_pos}, Sepsis-={n_neg})")
    ...
    data = {
        ...
        "metadata": {
            ...
            "n_pos_train": sum(1 for p in train_patients if p["sepsis3"] == 1),
            "n_pos_test": sum(1 for p in test_patients if p["sepsis3"] == 1),
            "train_ratio": train_ratio,
        },
    }

    out_file = out_path / f"sepsis_w{int(window_sec)}s.pt"
    torch.save(data, out_file)

    n_pos_train = data["metadata"]["n_pos_train"]
    n_pos_test = data["metadata"]["n_pos_test"]
    print(f"\n{'=' * 60}")
    print(f"  Sepsis Prediction — Data Prepared (Patient-Level)")
    print(f"  Output: {out_file}")
    print(f"  Train: {len(train_patients)} patients (sepsis+={n_pos_train})")
    print(f"  Test:  {len(test_patients)} patients (sepsis+={n_pos_test})")
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Sepsis Prediction — Data Preparation"
    )
    parser.add_argument("--cohort-csv", type=str, required=True)
    parser.add_argument("--waveform-dir", type=str, required=True)
    parser.add_argument("--out-dir", type=str, default="datasets/processed/sepsis")
