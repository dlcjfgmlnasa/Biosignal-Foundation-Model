# -*- coding:utf-8 -*-
"""ICU Mech Vent Need Prediction (vent_within_24h).
...
구조:
    ...
    → CLS output → LinearProbe → vent_need 예측
...
사용법:
    python -m downstream.outcome.vent_need.run \
        --checkpoint best.pt \
        --data-path datasets/processed/vent_need/vent_need_w600s.pt \
    ...
    for epoch in range(epochs):
        ...
        for batch_start in range(0, len(order), batch_size):
            ...
            if ddp_module is not None:
                ...
                labels = torch.tensor(
                    [p["vent_need"] for p in batch_patients],
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="ICU Mech Vent Need Prediction (Patient-Level Transformer Aggregation)"
    )
    ...
    if use_ddp and use_lora:
        ...
        def _extract_shard_tuples(patients):
            ...
            for gi in idxs:
                ...
                out.append(
                    (gi, reprs.detach().cpu(), p["vent_need"], str(p["subject_id"]))
                )
            return out
        ...
        train_patients = [
            {"vent_need": lbl, "subject_id": cid, "n_windows": r.shape[0]}
    ...
    print(f"\n{'=' * 60}")
    print(f"  ICU Mech Vent Need — {args.mode} (Transformer Aggregator)")
    print(f"{'=' * 60}")
    ...
    roc_path = out_dir / f"vent_need_roc_{args.mode}{fold_suffix}.png"
    plot_roc_curve(y_true, y_score, roc_path,
                   title=f"ICU Mech Vent Need — {args.mode} ROC")
    print(f"\nROC curve: {roc_path}")
    ...
    results_path = out_dir / f"vent_need_results_{args.mode}{fold_suffix}.json"
    with open(results_path, "w") as f:
        json.dump(results, f, indent=2, default=str)
    print(f"Results: {results_path}")

    npz_path = dump_fold_predictions(
        out_dir, task="vent_need", fold_idx=args.fold, n_folds=args.n_folds,
