#!/bin/bash
# ICU Mech Vent Need Prediction (#11) — patient-level Transformer Aggregator.
# Linear Probe + LoRA 두 모드 실행.
#
# 사용법:
#   # (1) 단일 GPU (기존 동작 — 결과 불변)
#   bash downstream/outcome/vent_need/bash/run.sh
#
#   # (2) B안 — torchrun+DDP: LoRA run 을 N GPU 로 데이터 병렬(환자 단위 분할).
#   NPROC=4 bash downstream/outcome/vent_need/bash/run.sh
...
DATA_DIR=${DATA_DIR:-/home/coder/workspace/k-mimic-/bio_fm/data/downstream/vent_need}
OUT_DIR=${OUT_DIR:-/home/coder/workspace/k-mimic-/bio_fm/result/main/vent_need}
DEVICE=${DEVICE:-cuda}
...
# Linear Probe.
LP_EPOCHS=${LP_EPOCHS:-500}
LP_LR=${LP_LR:-1e-3}
LP_BATCH=${LP_BATCH:-512}

# LoRA 설정.
LORA_EPOCHS=${LORA_EPOCHS:-500}
...
DATA_PATH="${DATA_DIR}/vent_need_w${WINDOW_SEC}s.pt"

echo "============================================================"
echo "  ICU Mech Vent Need (#11) — Transformer Aggregator"
...
$LAUNCH downstream.outcome.vent_need.run \
