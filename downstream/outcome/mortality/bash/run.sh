#!/bin/bash
# ICU Mortality Prediction — 실험 스크립트 (patient-level Transformer Aggregator)
...
#   bash downstream/outcome/mortality/bash/run.sh
#
#   # (2) B안 — torchrun+DDP: LoRA run 을 N GPU 로 데이터 병렬(환자 단위 분할).
#   #     NPROC>1 이면 lora 모드만 torchrun 으로 실행된다. linear_probe 는 frozen
#   #     feature 캐싱이라 DDP 이득이 없어 항상 단일 GPU.
#   NPROC=4 bash downstream/outcome/mortality/bash/run.sh
...
DATA_DIR=${DATA_DIR:-/home/coder/workspace/k-mimic-/bio_fm/data/downstream/mortality}
OUT_DIR=${OUT_DIR:-/home/coder/workspace/k-mimic-/bio_fm/result/main/mortality}
DEVICE=${DEVICE:-cuda}
...
$LAUNCH downstream.outcome.mortality.run \
