...
MIMIC-III Waveform + mortality cohort CSV →
ICU 사망 예측용 train/test .pt 파일 생성.

라벨: hospital_expire_flag (0=생존, 1=사망)

Mortality 전용 cohort (모든 ICU 유닛 포함, sepsis cohort는 5개 유닛만).
교차 일반화 스토리: 한국 데이터(VitalDB) pretrain → 미국 데이터(MIMIC-III) 평가.

사용법:
    python -m downstream.outcome.mortality.prepare_data \
        --cohort-csv downstream/classification/mortality/icu_mortality_cohort.csv \
        --waveform-dir datasets/raw/mimic3-waveform-mortality \
        --out-dir datasets/processed/mortality \
...
def load_cohort(cohort_csv: str) -> dict[int, dict]:
    ...
    Mortality 전용 cohort와 sepsis cohort 모두 지원한다.
    hospital_expire_flag를 mortality 라벨로 사용한다.
    """
...
def prepare_dataset(
    ...
    print(f"  Deaths: {sum(1 for p in patients.values() if p['mortality']==1)}")
    print(f"  Survivors: {sum(1 for p in patients.values() if p['mortality']==0)}")
    ...
    if max_patients is not None:
        # 사망 환자 우선 포함
        dead_sids = [s for s in subject_records if patients[s]["mortality"] == 1]
        alive_sids = [s for s in subject_records if patients[s]["mortality"] == 0]
    ...
    patient_data: list[dict] = []  # [{subject_id, mortality, windows: [signals]}]
    ...
    n_dead = sum(1 for p in patient_data if p["mortality"] == 1)
    n_alive = sum(1 for p in patient_data if p["mortality"] == 0)
    ...
    # 5. 환자 단위로 .pt 저장
    #    각 환자: windows (K, sig_type, win_samples), mortality label
    ...
    out_file = out_path / f"mortality_w{int(window_sec)}s.pt"
    torch.save(data, out_file)

    n_dead_train = data["metadata"]["n_dead_train"]
    n_dead_test = data["metadata"]["n_dead_test"]
    print(f"\n{'=' * 60}")
    print(f"  ICU Mortality Prediction — Data Prepared (Patient-Level)")
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="ICU Mortality Prediction — Data Preparation"
    )
    parser.add_argument(
        "--cohort-csv", type=str, required=True,
        help="sepsis3_cohort CSV (hospital_expire_flag 포함)",
    )
    parser.add_argument(
        "--waveform-dir", type=str, required=True,
        help="다운로드된 MIMIC-III waveform 디렉토리",
    )
    parser.add_argument("--out-dir", type=str, default="datasets/processed/mortality")
