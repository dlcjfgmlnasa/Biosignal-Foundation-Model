# -*- coding:utf-8 -*-
"""Sepsis Prediction (Sepsis-3 기준).

환자 단위 예측: 여러 10분 윈도우를 Foundation Model로 인코딩한 뒤,
Transformer Aggregator로 환자 수준 표현을 생성하여 sepsis 발생을 예측한다.

구조: Mortality task와 동일 (라벨만 sepsis3)
    윈도우 × K → Encoder (frozen) → h_1..h_K
    → [CLS] + h_1..h_K → Transformer Aggregator → CLS → Probe → sepsis 예측

사용법:
    python -m downstream.outcome.sepsis.run \
        --checkpoint best.pt \
        --data-path datasets/processed/sepsis/sepsis_w600s.pt \
        ...
        if cached_reprs_override is not None:
            ...
            for p in train_patients:
                reprs = encode_patient_windows(
                    ...
                    use_lora=False, session_prefix="sepsis",
                )
            ...
            if ddp_module is not None:
                ...
                for idx in batch_indices:
                    ...
                    if cached_reprs is not None:
                        ...
                        reprs = encode_patient_windows(
                            model, p, patch_size, max_windows,
                            use_lora=use_lora, session_prefix="sepsis",
                        )
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Sepsis Prediction (Patient-Level Transformer Aggregation)"
    )
    ...
    print(f"  Train: {len(train_patients)} patients "
          f"({n_pos_train} sepsis+, avg {avg_win_train:.1f} windows)")
    print(f"  Test:  {len(test_patients)} patients "
          f"({n_pos_test} sepsis+)")

    # ── DDP 분기 (mortality 와 동일 원칙) ──
    ...
    roc_path = out_dir / f"sepsis_roc_{args.mode}{fold_suffix}.png"
    ...
    results_path = out_dir / f"sepsis_results_{args.mode}{fold_suffix}.json"
    with open(results_path, "w") as f:
        json.dump(results, f, indent=2, default=str)
    print(f"Results: {results_path}")

    npz_path = dump_fold_predictions(
        out_dir, task="sepsis", fold_idx=args.fold, n_folds=args.n_folds,
