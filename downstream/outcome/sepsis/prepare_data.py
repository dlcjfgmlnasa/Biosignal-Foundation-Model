...
def prepare_dataset(
    ...
    sid_to_label: dict[int, int] = {p["subject_id"]: int(p["sepsis3"]) for p in patient_data}
    ...
    for fold_idx, (train_sids, val_sids, test_sids) in enumerate(splits):
        ...
        n_pos_val = sum(1 for p in val_patients if p["sepsis3"] == 1)
        ...
        out_file = out_path / f"sepsis_w{int(window_sec)}s{fold_suffix}.pt"
