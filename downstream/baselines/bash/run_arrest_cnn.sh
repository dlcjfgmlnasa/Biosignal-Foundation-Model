#!/bin/bash
# Cardiac Arrest end-to-end 1D-CNN baseline (SCOPE) — CARMEN 과 같은 데이터·5-fold·평가.
# 사전: cardiac_arrest prepare_data_scope 로 scope_{task}_{signals}_w{win}s_h{H}min_fold*.pt 생성.
# 사용법:
#   bash downstream/baselines/bash/run_arrest_cnn.sh
#   CUDA_VISIBLE_DEVICES=0 bash ...            # GPU 1장
#   HORIZONS=30 bash ...                        # 특정 horizon 만
#   env override: TASK/SIGNALS/WINDOW/HORIZONS/EPOCHS/PATIENCE/BATCH/LR/DATA_DIR/OUT_DIR

set -e

DATA_DIR=${DATA_DIR:-/home/coder/workspace/k-mimic-/bio_fm/data/downstream/cardiac_arrest}
OUT_DIR=${OUT_DIR:-/home/coder/workspace/k-mimic-/bio_fm/result/main/cardiac_arrest_cnn}
DEVICE=${DEVICE:-cuda}

TASK=${TASK:-arrest}                     # prepare_data_scope --task (arrest|death)
...
echo "============================================================"
echo "  Cardiac Arrest end-to-end 1D-CNN baseline (SCOPE)"
echo "  Data:    $DATA_DIR"
echo "  Output:  $OUT_DIR"
echo "  Task=$TASK Signals=$INPUT_SIGNALS Window=${WINDOW}s Horizons=$HORIZONS"
...
for H in $HORIZONS; do
    ...
    if ! ls "${PREFIX}"_fold0_*.pt >/dev/null 2>&1; then
        echo "[SKIP] ${PREFIX}_fold0_*.pt not found (prepare_data_scope 필요)"
        continue
    fi
    ...
    for f in $(seq 0 $((N_FOLDS - 1))); do
        ...
        echo -e "\n[${TASK} | w${WINDOW}s_h${H}min | fold ${f}]"
