#!/bin/bash
# Pulse-PPG frozen linear-probe baseline — PPG 전용 (ppg 채널만 사용, 50Hz).
# 사전: pulseppg 레포 + download_model.sh 로 받은 가중치(net 체크포인트).
# 사용법:
#   FM_PULSEPPG_ROOT=/repos/pulseppg WEIGHTS=/repos/pulseppg/.../pulseppg_weights.pt \
...
export ENCODER=pulseppg
export WEIGHTS=${WEIGHTS:?WEIGHTS 필요 (download_model.sh 로 받은 Pulse-PPG net 체크포인트)}
export THIRD_PARTY_ROOT=${THIRD_PARTY_ROOT:-${FM_PULSEPPG_ROOT:-}}
