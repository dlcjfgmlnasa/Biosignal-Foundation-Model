#!/bin/bash
# ST-MEM frozen linear-probe baseline — ECG 전용 (ecg 채널만, 250Hz/9s).
# 사전: ST-MEM 레포(models/) + Google Drive encoder-only 가중치.
# 사용법:
...
export ENCODER=stmem
export WEIGHTS=${WEIGHTS:?WEIGHTS 필요 (ST-MEM encoder-only 가중치, Google Drive)}
export THIRD_PARTY_ROOT=${THIRD_PARTY_ROOT:-${FM_STMEM_ROOT:-}}
