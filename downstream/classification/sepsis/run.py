...
사용법:
    python -m downstream.classification.sepsis.run \
        --checkpoint best.pt \
...
# Mortality의 TransformerAggregator와 헬퍼 함수 재사용
from downstream.classification.mortality.run import (
    TransformerAggregator,
    _make_samples_for_window,
    _mean_pool,
    _collate_patients,
    ...
    for epoch in range(epochs):
        ...
        for batch_start in range(0, len(order), batch_size):
            ...
            for idx in batch_indices:
                ...
                if use_lora:
                    reprs = _encode_with_grad(model, p, patch_size, max_windows)
                else:
...
def main() -> None:
    ...
    roc_path = out_dir / f"sepsis_roc_{args.mode}.png"
    ...
    results_path = out_dir / f"sepsis_results_{args.mode}.json"
