# -*- coding:utf-8 -*-
"""Ventilation Quality Classification (ETCO2 기반 2-class).
...
입력: CO2 + AWP waveform → Foundation Model encoder → mean pool → LinearProbe
라벨: ETCO2 < 35 → Hyperventilation(0), ETCO2 >= 35 → Normal(1)

사용법:
    # Linear probe
    python -m downstream.classification.ventilation_quality.run \
        --checkpoint best.pt --mode linear_probe \
        --data-path data/downstream/ventilation_quality/ventilation_quality_co2_awp.pt
...
class MultiSignalWindow:
    ...
    signals: dict[str, np.ndarray]  # {"co2": (win_samples,), "awp": ...}
    label: int
    label_value: float  # ETCO2 값
    case_id: str | int
...
def _multi_window_to_samples(mw: MultiSignalWindow, idx: int) -> list[BiosignalSample]:
    ...
    for ch, (sig_type, signal) in enumerate(mw.signals.items()):
        stype_int = SIGNAL_TYPES.get(sig_type, 4)  # co2=4, awp=5
...
def _load_data(args) -> tuple[list[MultiSignalWindow], list[MultiSignalWindow]]:
    """prepared .pt에서 train/test MultiSignalWindow를 반환한다."""
    ...
    def _pt_to_windows(split_data):
        ...
        etco2_t = split_data["etco2_values"]
        n = len(labels_t)
        for i in range(n):
            signals = {
            ...
            windows.append(
                MultiSignalWindow(
                    ...
                    label_value=float(etco2_t[i].item()),
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Ventilation Quality Classification (2-class)"
    )
    ...
    n_hyper_train = sum(1 for w in train_windows if w.label == 0)
    n_normal_train = sum(1 for w in train_windows if w.label == 1)
    n_hyper_test = sum(1 for w in test_windows if w.label == 0)
    n_normal_test = sum(1 for w in test_windows if w.label == 1)
    print(
        f"  Train: {len(train_windows)} samples "
        f"(Hyper={n_hyper_train}, Normal={n_normal_train})"
    )
    print(
        f"  Test:  {len(test_windows)} samples "
        f"(Hyper={n_hyper_test}, Normal={n_normal_test})"
    )
    ...
    roc_path = out_dir / f"ventilation_roc_{args.mode}.png"
    plot_roc_curve(
        y_true, y_score, roc_path,
        title=f"Ventilation Quality — {args.mode} ROC",
    )
    ...
    results_path = out_dir / f"ventilation_results_{args.mode}.json"
