# -*- coding:utf-8 -*-
"""Task 1: EtCO2 Abnormal Prediction (EtCO2 <35 | >45 mmHg, >=1min sustained).
...
사용법:
    # Linear probe (기본)
    python -m downstream.acute_event.etco2_abnormal.run \
...
def _load_data(
    ...
    if args.data_path:
        ...
        from downstream.acute_event.etco2_abnormal.prepare_data import (
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Task 1: EtCO2 Abnormal Prediction")
    ...
    plot_roc_curve(
        ...
        title=f"Task 1: EtCO2 Abnormal — {args.mode} ROC",
    )
    ...
    # fold suffix: png 와 일치(n_folds>1 동시 실행 시 충돌 방지, n_folds=1 면 불변).
    results_path = out_dir / f"task1_results_{args.mode}{_roc_suffix}.json"
