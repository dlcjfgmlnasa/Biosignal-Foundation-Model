#!/bin/bash
# Cardiac Arrest Prediction — 실험 스크립트 (patient-level Transformer Aggregator)
...
#   #     NPROC>1 이면 lora 모드만 torchrun. linear_probe 는 frozen feature 캐싱이라
#   #     DDP 이득이 없어 항상 단일 GPU.
#   NPROC=4 bash downstream/acute_event/cardiac_arrest/bash/run.sh
...
DATA_PATH="${DATA_DIR}/cardiac_arrest_w${WINDOW_SEC}s.pt"
...
$LAUNCH downstream.acute_event.cardiac_arrest.run \
