# -*- coding:utf-8 -*-
"""Desaturation Prediction (SpO2 < 90%, >=1min sustained) — SNUH ventilation_eventcases.
...
사용법:
    # Linear probe (기본)
    python -m downstream.acute_event.desaturation.run \
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Desaturation Prediction (SNUH)")
    ...
    plot_roc_curve(
        ...
        title=f"Desaturation — {args.mode} ROC",
    )
    ...
    npz_path = dump_fold_predictions(
        out_dir, task="desaturation", fold_idx=args.fold, n_folds=args.n_folds,
