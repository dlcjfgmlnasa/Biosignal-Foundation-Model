# -*- coding:utf-8 -*-
"""Hypoxemia Prediction (SpO2 < 90%, >=1min sustained)  - Paper 4.2.2.

VitalDB SpO2 numeric track 기반 저산소혈증 예측. PPG waveform을 입력으로 사용.
...
사용법:
    # Dummy (파이프라인 검증)
    python -m downstream.acute_event.hypoxemia.run --dummy

    # Linear probe
    python -m downstream.acute_event.hypoxemia.run \
        --checkpoint best.pt --mode linear_probe \
        --data-path outputs/downstream/hypoxemia/hypoxemia_vitaldb_ppg_h5min.pt
...
def _load_data(
    ...
    """Prepared .pt 파일 → MultiSignalWindow 리스트.

    .pt 스키마 (prepare_data.py로 생성):
        {
            "metadata": {"task": "hypoxemia", "horizon_sec": 300, ...},
            "train": {"signals": {st: (N, win)}, "labels": (N,), "label_values": (N,), "case_ids": (N,)},
            "test":  {...}
        }
    """
    print(f"\nLoading: {data_path}")
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Hypoxemia Prediction (SpO2<90%)")
    ...
    print(f"  Train: {len(train_windows)} ({n_pos_tr} hypoxemia)")
    print(f"  Test:  {len(test_windows)} ({n_pos_te} hypoxemia)")
    ...
    roc_path = out_dir / f"hypoxemia_roc_{args.mode}.png"
    plot_roc_curve(y_true, y_score, roc_path, title=f"Hypoxemia  - {args.mode}")
    print(f"\nROC: {roc_path}")
    ...
    results_path = out_dir / f"hypoxemia_results_{args.mode}.json"
