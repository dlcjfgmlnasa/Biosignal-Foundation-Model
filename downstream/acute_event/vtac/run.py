# -*- coding:utf-8 -*-
"""False VT Alarm Reduction (VTaC) — binary detection (True VT vs False alarm).

PhysioNet VTaC 1.0. 알람 주변 15초(retrospective, [alarm-10s, alarm+5s]) 윈도우 →
Foundation Model encoder → mean pool → LinearProbe 로 진짜 VT(1) vs 오알람(0) 판별.
...
데이터: ``downstream.acute_event.vtac.prepare_data`` 산출 fold 파일
(``vtac_<mode>_fold{i}.pt`` = {train,val,test,metadata}, event-level binary).

사용법:
    # Linear probe (fold 0)
    python -m downstream.acute_event.vtac.run \
        --checkpoint best.pt --model-version v2 --mode linear_probe \
        --data-path outputs/downstream/vtac/vtac_ecg_ppg --fold 0 --n-folds 5

    # LoRA fine-tuning
    python -m downstream.acute_event.vtac.run \
        --checkpoint best.pt --model-version v2 --mode lora --lr 1e-4 --epochs 30 \
        --data-path outputs/downstream/vtac/vtac_ecg_ppg --fold 0 --n-folds 5
"""
...
def _load_data(
    ...
    # prepare_data 는 self-contained per-fold 파일(vtac_<mode>_fold{i}.pt)을 낸다.
    # --data-path 가 직접 fold 파일이면 그대로, prefix 면 _fold{fold}.pt 를 붙인다.
    dp = Path(args.data_path)
    if dp.is_file():
        fold_path = dp
    else:
        fold_path = Path(f"{args.data_path}_fold{int(args.fold)}.pt")
    print(f"\nLoading prepared data: {fold_path}")
    data = load_prepared_split_chunked(fold_path, fold=None)
    ...
    print(
        ...
        f"Window: {meta.get('win_sec', '?')}s, split_mode={meta.get('split_mode', '?')}"
    )
    ...
    def _pt_to_windows(split_data):
        # VTaC 스키마: labels(0/1), patients(record=cluster id), segment_ids(event).
        # label_value 는 연속값이 없어 label 을 그대로 쓴다.
        windows = []
        labels_t = split_data["labels"]
        patients = split_data.get("patients")
        seg_ids = split_data.get("segment_ids")
        n = len(labels_t)
        for i in range(n):
            signals = {
            ...
            windows.append(
                MultiSignalWindow(
                    signals=signals,
                    label=lab,
                    label_value=float(lab),
                    # case_id = record → patient-cluster bootstrap 의 grouping id.
                    case_id=patients[i] if patients is not None
                    else (seg_ids[i] if seg_ids is not None else i),
                )
            )
        return windows
    ...
    if val_data is not None:
        ...
        # Backward-compat: val 키 없으면 train 에서 20% 동적 split.
        warnings.warn(
...
def main() -> None:
    parser = argparse.ArgumentParser(description="VTaC False VT Alarm (binary)")
    ...
    parser.add_argument(
        ...
        default=["ecg", "ppg"],
        choices=["ecg", "ppg", "abp"],
        help="Input signal types (VTaC: ECG+PPG 기본, ABP 는 소수 event 만 보유). "
             "실제 사용 신호는 prepared-data 파일에서 결정됨.",
    )
    parser.add_argument(
        "--val-split-seed",
        type=int,
    ...
    def _pos_stats(name: str, ws: list[MultiSignalWindow]) -> None:
        ...
        print(
            ...
            f"({n_pos} True-VT, {n_pos / max(len(ws), 1) * 100:.1f}%)"
        )
    ...
    roc_path = out_dir / f"vtac_roc_{args.mode}{_roc_suffix}.png"
    plot_roc_curve(
        y_true, y_score, roc_path,
        title=f"VTaC False VT Alarm — {args.mode} ROC",
    )
    ...
    results_path = out_dir / f"vtac_results_{args.mode}{fold_suffix}.json"
    ...
    npz_path = dump_fold_predictions(
        out_dir, task="vtac", fold_idx=args.fold, n_folds=args.n_folds,
